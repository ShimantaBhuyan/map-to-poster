	...
	if (isArtistic) {
		...
		if (artisticMap && artisticContainer) {
			try {
				...
				if (state.showMarker && state.markers && state.markers.length > 0) {
					...
					const color = theme.route || theme.road_primary || theme.text || '#0f172a';
		...
		try {
			...
			if (state.showMarker && state.markers && state.markers.length > 0) {
				...
				const color = theme.route || theme.text || theme.textColor || '#0f172a';
			...
			if (state.showRoute) {
				...
				drawComplexRouteToCtx(ctx, points, theme.route || '#EF4444', themeBg);
			}
