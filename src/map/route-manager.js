...
export function updateRouteStyles(currentState) {
	const map = getMap();
	...
	const casingColor = isArtistic ? (theme.bg || '#ffffff') : (theme.background || theme.backgroundColor || '#ffffff');

	if (currentState.showRoute) {
		const start = [currentState.routeStartLat, currentState.routeStartLon];
		const end = [currentState.routeEndLat, currentState.routeEndLon];
		...
		const routeCoords = (currentState.routeGeometry && currentState.routeGeometry.length > 0)
			? currentState.routeGeometry.map(c => [c[1], c[0]])
